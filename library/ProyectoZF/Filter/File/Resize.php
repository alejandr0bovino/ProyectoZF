 ...
 */
class ProyectoZF_Filter_File_Resize implements Zend_Filter_Interface
{
    ...
    protected $_adapter = 'ProyectoZF_Filter_File_Resize_Adapter_Gd';
     ...
     * @return ProyectoZF_Filter_File_Resize An instance of this filter
     */
    public function __construct($options = array())
    {
        ...
        if (isset($options['adapter'])) {
            if ($options['adapter'] instanceof ProyectoZF_Filter_File_Resize_Adapter_Abstract) {
                $this->_adapter = $options['adapter'];
            } else {
                $name = $options['adapter'];
                if (substr($name, 0, 33) != 'ProyectoZF_Filter_File_Resize_Adapter_') {
                    $name = 'ProyectoZF_Filter_File_Resize_Adapter_' . ucfirst(strtolower($name));
                }
                $this->_adapter = $name;
            }
        }
    ...
    {
        if ($this->_adapter instanceof ProyectoZF_Filter_File_Resize_Adapter_Abstract) {
            return;
        } else {
